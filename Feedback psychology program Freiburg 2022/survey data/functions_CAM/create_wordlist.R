...
create_wordlist <- function(dat_nodes = CAMfiles[[1]],
  ...
  colnames(freq_terms) <- c("labels", "raw","percent")
  ...
  for(i in 1:length(freq_terms$labels)){
    tmp <- dat_nodes$value[dat_nodes$text == freq_terms$labels[i]]

    ## valence mean / sd
    freq_terms$mean_valence[freq_terms$labels == freq_terms$labels[i]] <- mean(tmp)
    freq_terms$sd_valence[freq_terms$labels == freq_terms$labels[i]] <- sd(tmp)

    ## degree mean / sd
    freq_terms$mean_degree[freq_terms$labels == freq_terms$labels[i]] <-
      mean(tmp_dat_out$degree[tmp_dat_out$label == freq_terms$labels[i]])
    freq_terms$sd_degree[freq_terms$labels == freq_terms$labels[i]] <-
      sd(tmp_dat_out$degree[tmp_dat_out$label == freq_terms$labels[i]])



    ## if comments TRUE
    if(comments){
    tmp_comments <- dat_nodes$comment[dat_nodes$text == freq_terms$labels[i]]
    if(tmp_comments != "" && !is.na(tmp_comments) && !identical(tmp_comments, character(0))){
